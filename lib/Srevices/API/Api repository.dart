...
//-----------------------------------------------------------------------------------------------------------------
  static Future<Movies> fetchSearch(String queryParameters) async {
    Uri url = Uri.https( 'api.themoviedb.org'      ,   '/3/search/movie', {'api_key': apiKey, 'query': queryParameters,},);
    final response = await http.get(url);     //get data
    var searchResponse = Movies.fromJson(jsonDecode(response.body)); // convert data
    if (response.statusCode == 200) {  //check data
...
//---------------------------------------------------------------------------------------------------------------------------------------
  ...
  static Future<Movies> fetchMoviesData() async {     //get movies data
    ...
    var response = await http.get(Uri.parse('https://api.themoviedb.org/3/movie/top_rated?api_key=${apiKey}&language=en-US&page=11 '),);
...
//---------------------------------------------------------New release , show --------------------------------------------------------
  static Future<Movies> fetchPopular() async {    //get popular
    final response = await http.get(Uri.parse('https://api.themoviedb.org/3/movie/popular?api_key=${apiKey}&language=en-US&page=6'),);
...
//---------------------------------------------------------------------- -- -------------------------------------------------------------
  static Future<Movies> fetchSimilar(int movieId) async {
    ...
    Uri url = Uri.https('api.themoviedb.org', header, {'api_key': apiKey,},);    //data link
    final response = await http.get(url, headers: {'movie_id': '',},); // get data
